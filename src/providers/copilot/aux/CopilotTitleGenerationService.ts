...
import { resolveCopilotRuntimeModel } from '../runtime/CopilotChatRuntime';
...
export class CopilotTitleGenerationService implements TitleGenerationService {
  ...
  async generateTitle(
    ...
    const runner = new CopilotAuxQueryRunner(this.plugin);
  ...
  private resolveTitleModel(): string {
    ...
    const providerModel = typeof providerSettings.model === 'string'
      ...
      : undefined;
    const configuredTitleModel = this.plugin.settings.titleGenerationModel || undefined;
    return resolveCopilotRuntimeModel(configuredTitleModel, providerModel);
  }
